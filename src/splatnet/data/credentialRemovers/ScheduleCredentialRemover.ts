...
class ScheduleCredentialRemover {
    removeAllCredentials(stageSchedule: StageScheduleQuery_730cd98): StageSchedule {
        const bankara = this.removeBankaraScheduleCredentials(stageSchedule.bankaraSchedules);
        const regular = this.removeRegularScheduleCredentials(stageSchedule.regularSchedules);
        const salmon = this.removeSalmonRunScheduleCredentials(stageSchedule.coopGroupingSchedule);
        const x = this.removeXScheduleCredentials(stageSchedule.xSchedules);
        const league = this.removeLeagueScheduleCredentials(stageSchedule.leagueSchedules);
    ...
    removeBankaraScheduleCredentials(bankaraSchedule: StageScheduleQuery_730cd98['bankaraSchedules']) {
        return this._parseBankaraSchedules(bankaraSchedule);
    }

    removeRegularScheduleCredentials(regularSchedule: StageScheduleQuery_730cd98['regularSchedules']) {
        return this._parseSchedules(regularSchedule, 'regular');
    }

    removeXScheduleCredentials(xSchedule: StageScheduleQuery_730cd98['xSchedules']) {
        return this._parseSchedules(xSchedule, 'x');
    }

    removeLeagueScheduleCredentials(leagueSchedule: StageScheduleQuery_730cd98['leagueSchedules']) {
    ...
    private _parseSchedules(anySchedules: any, matchType: 'regular' | 'bankara' | 'x' | 'league'): Schedule[] {
        const nodes = anySchedules['nodes'];

        const schedules = nodes.map((node: any) => {
            const settingsPropertyName =
                matchType === 'bankara' ? `${matchType}MatchSettings` : `${matchType}MatchSetting`;
    ...
    private _parseBankaraSchedules(bankaraSchedulesJson: StageScheduleQuery_730cd98['bankaraSchedules']): {
        open: Schedule[];
        challenge: Schedule[];
    } {
