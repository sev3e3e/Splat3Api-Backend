...
import { splatnet3ApiClient } from '../../SplatNet3Client.js';
...
export class StageScheduleUpdater extends DataUpdater {
    ...
    async update(): Promise<StageSchedule> {
        const data = await splatnet3ApiClient.getAllSchedules();

        if (data == undefined) {
            throw new Error('failed to get schedules');
        }

        const regularSchedules = this._parseSchedules(data['data']['regularSchedules'], 'regular');
        const bankaraSchedules = this._parseBankaraSchedules(data['data']['bankaraSchedules']);
        const xSchedules = this._parseSchedules(data['data']['xSchedules'], 'x');
        const leagueSchedules = this._parseSchedules(data['data']['leagueSchedules'], 'league');
